...
def split_data_for_calendar_analysis(data, start_date, end_date, effect_period):
    """
    Splits the data into train and test sets based on the calendar effect period.
    ...
    :param effect_period: Tuple containing start and end months for the effect period.
    ...
    # Split data based on effect period
    effect_start, effect_end = effect_period
    train_data = filtered_data[~((filtered_data.index.month >= effect_start) & (filtered_data.index.month <= effect_end))]
    test_data = filtered_data[((filtered_data.index.month >= effect_start) & (filtered_data.index.month <= effect_end))]

    return train_data, test_data


def main():
    ticker_list = ['AAPL', 'GOOG', 'MSFT', 'NDX', 'NYA']
